...
def predict(image_path, model_path, img_size=640, iou_threshold=0.3, score_threshold=0.4, class_names=None):
    ...
    print("Final bounding boxes after NMS:")
    for i, (box, score, cls) in enumerate(zip(boxes, scores, classes), start=1):
        print(f"Box {i}: {box}, Score: {score}, Class: {cls}")

    # Draw boxes
    print("Drawing bounding boxes on the image...")
    image_with_boxes = draw_boxes(original_image, boxes, scores, classes, class_names)
    kitsune = cv2.resize(image_with_boxes, (500,600))
    cv2.imshow("Detection Results", kitsune)
    cv2.waitKey(0)
    cv2.destroyAllWindows()

# Call the predict function with defined variables
predict(image_path, model_path, img_size, iou_threshold, score_threshold, class_names)
